...
[SlipAngles,LatAccelG,Betamax,YawVelo,LateralVelo] = SlipAngleSim(SteerAngles,BetaEntry,Velocity,LatAccelG,Radius,vehicleObj);
...
[Fz,LLT,LLT_D,R_g,Roll_Angle,Z] = LLTSim(K_roll,AccelEntry(1,2),vehicleObj);

[IA] = CamberSim(Roll_Angle,SWAngle,vehicleObj);

mu = [Fy(1,1)/Fz(1,1), Fy(1,2)/Fz(1,2); Fy(2,1)/Fz(2,1), Fy(2,2)/Fz(2,2)];
...
% Input Steering Wheel Angle, CoG Slip Angle
SWAngle = 2; %linspace(-90,90,5); %deg (L = neg, R = pos)
BetaApex = -5; %linspace(-10,10,5); %CoG slip angle (deg) (neg -> Right, pos -> Left)

SteerAngles = SteerAngleSim(SWAngle,vehicleObj);
          
[SlipAngles,LatAccelG,Betamax,YawVelo,LateralVelo] = SlipAngleSim(SteerAngles,BetaApex,Velocity,AccelEntry(1,2),Radius,vehicleObj);
...
[Fz,LLT,LLT_D,R_g,Roll_Angle,Z] = LLTSim(K_roll,Accel(1,2),vehicleObj);
